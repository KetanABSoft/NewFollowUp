...
class _SetWidgetsState extends State<SetWidgets> {
      ...
      builder: (BuildContext context) {
        if (message == 'success' && data != null) {
          return AlertDialog(
            ...
            content: Text(
              "Hourly Rate: ${data["hourlyRate"].toString()}",
            ),
            actions: <Widget>[
              TextButton(
                child: Text("OK"),
                    ...
                    Column(
                      children: [
                        GestureDetector(
                          onTap: () {
                            print("Hiii");
                            Add(); // Call Add method when Calculate button is tapped
                          },
                          child: Container(
                            height: 50,
                            width: 350,
  ...
  Future<void> Add() async {
    ...
    final Response response = await http.post(
      Uri.parse(
          "http://103.159.85.246:4000/api/salary/calculate-hourly-wage"),
      body: jsonEncode(abc),
      headers: {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        showResponseDialog('success'); // Show dialog on success
        storeHourlyRate(data["hourlyRate"]);
      });
    } else {
      print('Error: ${response.statusCode}');
      showResponseDialog('Error: ${response.statusCode}');
    }
  }
